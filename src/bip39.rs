...
pub fn to_bip39_secret(data: &[u8]) -> VarU16_255 {
    ...
    for i in 0..len {
        ...
        for j in 0..11 {
            let bit_pos = i * 11 + j;
            let bit = if bit_pos < data.len() * 8 {
                (data[bit_pos / 8] >> (7 - (bit_pos % 8))) & 1
            } else {
                let cs_pos = bit_pos - data.len() * 8;
    ...
    VarU16_255::from_u16_arr(&buf[..len])
}

pub fn from_bip39_secret(data: &[u16]) -> VarByte255 {
    // Trust the caller right now for simplicity
    assert!(data.len() % 3 == 0 && data.len() <= 189);

    let byte_len = data.len() * 4 / 3;
    let mut value = [0u8; 256];
    value[0] = byte_len as u8;

    for i in 0..byte_len {
        let mut byte_val = 0u8;
        for j in 0..8 {
            let bit_pos = i * 8 + j;
            let bit = (data[bit_pos / 11] >> (10 - (bit_pos % 11))) & 1;
...
pub fn from_bip39_mnemonic(mnemonic: &VarString511) -> VarU16_255 {
    ...
    for word in mnemonic.to_str().split(' ') {
        if let Some(idx) = BIP39_WORDLIST.iter().position(|&w| w == word) {
            arr[len] = idx as u16;
            len += 1;
        }
    }
